...
export const SUPPLY_FORCES = [
  { key: 'accel',
    story: "Inference is escaping the general-purpose GPU. Rubin-class parts claim ~10× lower cost per generated token, and custom inference ASICs grow from ~15% of the market in 2024 to ~40% in 2026. Cheap-and-slow and expensive-and-instant are splitting into separately priced lanes.", name: 'Accelerator price-performance', low: 2, base: 3.0, high: 4, color: 'var(--accent-cyan)',
  ...
  { key: 'arch',
    story: "The centre of gravity moves to small models. A 7B model serves routine agent steps 10–30× cheaper than a 70B generalist, and most steps inside a deployed agent pipeline are routine. Gartner expects task-specific small models used at 3× the rate of general LLMs by 2027.", name: 'Model architecture efficiency', low: 1.9, base: 2.8, high: 3.5, color: 'var(--accent-violet)',
    ...
    confidence: 'Medium' },
  { key: 'software',
    story: "The unglamorous half of the decline. Continuous batching already delivered ~85% and is baked into today's prices; what remains is speculative decoding, disaggregated prefill/decode and prefix reuse — pure software, no new silicon.", name: 'Serving software', low: 1.35, base: 1.6, high: 1.9, color: 'var(--accent-green)',
    ...
    confidence: 'Medium' },
  { key: 'numerics',
    story: "Decoding is memory-bound, so the memory roadmap is the cost roadmap. HBM4 doubles interface width and lifts every node's ceiling — but two vendors control ~90% of supply and prices rose ~20%, so declines are gated by memory as much as by GPU launches.", name: 'Numerics & quantisation', low: 1.25, base: 1.5, high: 1.8, color: 'var(--accent-orange)',
    ...
    confidence: 'Medium-high' },
  { key: 'margin',
    story: "Competition, not engineering. Open-weight releases put a hard ceiling on every matched tier: DeepSeek V4 priced ~97% below frontier, and matched tiers have been observed collapsing to hosting economics within weeks.", name: 'Margin & market structure', low: 1.15, base: 1.3, high: 1.45, color: 'var(--accent-pink)',
    ...
    confidence: 'Low' },
  { key: 'capex',
    story: "Getting more out of the same silicon. Hyperscalers extended server depreciation to 5–6 years, and one frontier lab's inference gross margin moved from under 40% to over 70% within a year — evidence of large, real utilisation gains.", name: 'Capex amortisation & utilisation', low: 1.05, base: 1.2, high: 1.3, color: 'var(--accent-yellow)',
    ...
    confidence: 'Medium' },
  { key: 'energy',
    story: "The one everybody talks about and almost nobody should. Nuclear PPAs and SMR orders anchor supplier cost structures into the 2030s — but energy contributes only ~3%. It constrains where capacity can be built; it is not what makes tokens cheap.", name: 'Energy & datacentre', low: 0.95, base: 1.1, high: 1.2, color: 'var(--text-dim)',
...
export const DEMAND_FORCES = [
  { key: 'penetration',
    story: "The single biggest demand force, and the slowest. Goldman assumes only 12% of knowledge workers use agentic AI by 2030 (37% by 2040) — that long adoption tail is precisely what keeps the forecast at 24× rather than the triple-digit multiples often quoted.", name: 'Workload penetration', low: 3, base: 5.2, high: 8, color: 'var(--accent-pink)',
    ...
    confidence: 'Medium' },
  { key: 'agentic',
    story: "Agents re-send everything, every step. A 20-step agent pays for its system prompt twenty times, and the context grows at each step. Gartner measures 5–30× per task; this figure is the blended effect across all traffic, not the per-task peak.", name: 'Agentic task structure', low: 1.8, base: 3.0, high: 4.5, color: 'var(--accent-violet)',
    ...
    confidence: 'Medium' },
  { key: 'reasoning',
    story: "You pay for thinking you never see. Reasoning models generate hidden chain-of-thought before the visible answer, billed at the full output rate — discounted here because the underlying forecast already embeds some reasoning overhead.", name: 'Reasoning tokens', low: 1.2, base: 1.45, high: 1.8, color: 'var(--accent-orange)',
    ...
    confidence: 'Medium' },
  { key: 'multimodal',
    story: "Images, audio and video carry far more tokens per interaction than text. This is the weakest-sourced term in the model — challenge it first if you are pressure-testing the number.", name: 'Multimodality', low: 1.1, base: 1.25, high: 1.6, color: 'var(--accent-cyan)',
    what: 'Image, audio and video tokens per interaction',
    basis: 'Judgement-based — no direct source. Challenge this one first.',
    confidence: 'Low' },
  { key: 'context',
    story: "Million-token windows invite teams to stuff entire repositories into every call. Longer windows, richer retrieval and larger tool payloads all push the same direction, and input is billed linearly.", name: 'Context growth per call', low: 1.1, base: 1.22, high: 1.45, color: 'var(--accent-yellow)',
...
export const DRAG_FORCE = {
  key: 'drag',
    story: "The only force that removes billable tokens — and the only one a single company controls. Market average is 1.45×; 3× is well within reach of caching, routing and compression applied seriously. Push your drag and you halve your own growth while consuming the same intelligence.", name: 'Efficiency & optimisation drag', low: 1.1, base: 1.45, high: 3,
